...
local options = {
    ...
    font = 0,
    menu_title = "NPC",
    menu_subtitle = "Choisissez votre cargaison",
    color_r = 0,
    color_g = 128,
    color_b = 255,
}
	...
	while true do
		...
		local v = Config.CargoProviderLocation 
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Appuyez sur ~INPUT_CONTEXT~ pour parler au ~y~Gérant des cargaisons")
			...
			if event_is_running then
				...
				if pVehicle == event_vehicle then
					...
					if delivery_point_distance < 50.0 then
					 ...
					 DrawMissionText("Retourner dans le véhicule de livraison !", 1000)
				end
			end
	end
end)
		...
		Citizen.Wait(5000)
				if event_is_running then
						...
						if IsPedDeadOrDying(GetPlayerPed(-1)) then
							ResetCargo()
							DisplayMissionFailed('Vous êtes mort ! Votre mission s\'arrête la.')
						end

						if GetVehicleEngineHealth(event_vehicle) < 20 and event_vehicle ~= nil then
							ResetCargo()
							DisplayMissionFailed('La cargaison a était sérieusement endommager !')
						end

						if event_time_passed > 1800 then
							ResetCargo()
							DisplayMissionFailed('Votre cargaison ne sera pas payer ! Vous avez mis trop de temps.')
						end
...
function CargoMenu()
	ClearMenu()
    options.menu_title = "   Gérant des cargaisons"
    for i = 1, #Config.Scenarios do
    	Menu.addButton("Acheter cargaisons - $" .. Config.Scenarios[i].CargoCost, "PurchaseCargo", i)
   	end
    Menu.addButton("Arreter de discuter","CloseMenu",nil) 
end
...
function AlertThePolice()
	...
	if event_is_running then
    	...
    	TriggerServerEvent('esx_addons_gcphone:startCall', 'Brigade Anti-Drogue', ">ANONYMOUS< Ta cargaison on va la choppé - [ " .. vehicle_plate .. " ] - Nos indic nous ont dit ta plaque, tu es foutu !", PlayerCoords, {
...
function PurchaseCargo(scenario)
	...
	if event_is_running == true then
		...
		drawNotification("Livrez la cargaison au plus vite ou vous ne serrez pas payer !")
		goto done
	...
	ESX.TriggerServerCallback('esx_cargodelivery:getCopsOnline', function(police)
		...
		if police >= Config.Scenarios[scenario].MinCopsOnline then
			...
			if bought then
				...
				drawNotification("Votre cargaison doit être livré en moin de 30 minutes !!")
			...
			drawNotification("Vous avez besoin de ~b~" .. Config.Scenarios[scenario].MinCopsOnline .. " flics [ Non Moldu ] ~w~en ville.")
