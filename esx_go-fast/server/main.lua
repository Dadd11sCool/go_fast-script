...
ESX.RegisterServerCallback('esx_cargodelivery:sellCargo', function(source, cb, price)
	...
	TriggerClientEvent('esx:showNotification', source, "Vous avez gagné ~r~" .. price .. "€~w~ grace à cette livraison, c\'est 5 fois plus qu'à l'achat !")
	cb(true)
...
ESX.RegisterServerCallback('esx_cargodelivery:buyCargo', function(source, cb, price)
	...
	if (os.time() - LastDelivery) < 200.0 and LastDelivery ~= 0.0 then
		...
		TriggerClientEvent('esx:showNotification', source, "Cargaison en cour !")
		cb(false)
		...
		if Config.UsesBlackMoney then
			...
			if xPlayer.getAccount('black_money').money >= price then
				...
				TriggerClientEvent('esx:showNotification', source, "Vous n\'avez pas ~r~d\'argent sale~w~ !!")
				...
				TriggerClientEvent('esx:showNotification', source, "Vous n\'avez pas ~r~d\'argent~w~!!!")
